...
import { FinalCTADecorations, AnimatedSparkles, AnimatedHeadline } from "./FinalCTADecorations.client";
import {
  siteConfig,
  localizedPath,
  withLocalePrefix,
...
export async function FinalCTA({ lang }: { lang: string }) {
  ...
  return (
    <section className="relative overflow-hidden py-12 sm:py-16 md:py-20 lg:py-24 xl:py-28 hero-section-bg z-60">
        ...
        <div className="max-w-6xl mx-auto text-center px-4 sm:px-6">
          <div className="flex justify-center mb-6 sm:mb-8">
            <div className="px-5 py-2.5 sm:px-6 sm:py-3 bg-foreground/5 dark:bg-white/20 backdrop-blur-lg rounded-full text-sm sm:text-base font-bold text-foreground dark:text-white flex items-center gap-2 border border-border dark:border-white/35 shadow-xl hover:scale-105 transition-transform duration-300">
              <AnimatedSparkles />
              <span className="whitespace-nowrap bg-gradient-to-r from-yellow-400 to-orange-500 bg-clip-text text-transparent font-extrabold">{c.badge}</span>
              <div className="w-1.5 h-1.5 rounded-full bg-[hsl(var(--gold))] animate-pulse"></div>
            </div>
          </div>

          <AnimatedHeadline className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-extrabold mb-6 sm:mb-8 md:mb-10 text-foreground leading-[1.1]">
